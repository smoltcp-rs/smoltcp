...
use wire::{IPV6_MIN_MTU, Ipv6Address, Ipv6Packet, Ipv6Repr};
#[cfg(feature = "proto-ipv4")]
use wire::{IPV4_MIN_MTU, Ipv4Address, Ipv4Packet, Ipv4Repr};
#[cfg(feature = "proto-ipv4")]
...
use socket::{AnySocket, Socket, SocketSet};
    ...
    DeviceT: for<'d> Device<'d>,
{
    ...
    pub fn neighbor_cache(
        mut self, neighbor_cache: NeighborCache<'b>
    ...
    DeviceT: for<'d> Device<'d>,
{
    ...
    where
        GatewayAddrT: Into<Option<Ipv4Address>>,
    {
    ...
    pub fn poll_at(&self, sockets: &SocketSet, timestamp: Instant) -> Option<Instant> {
        sockets
            ...
            .filter_map(|socket| {
                ...
                socket
                    .meta()
                    .poll_at(socket_poll_at, |ip_addr| self.inner.has_neighbor(&ip_addr, timestamp))
            })
            .min()
    }
    ...
    fn socket_egress(&mut self, sockets: &mut SocketSet, timestamp: Instant) -> Result<bool> {
        ...
        for mut socket in sockets.iter_mut() {
            ...
            match (device_result, socket_result) {
                ...
                (Err(err), _) | (_, Err(err)) => {
                    net_debug!("{}: cannot dispatch egress packet: {}", socket.meta().handle, err);
                    return Err(err);
                }
...
impl<'b, 'c> InterfaceInner<'b, 'c> {
    ...
    fn raw_socket_filter<'frame>(
        &mut self, sockets: &mut SocketSet, ip_repr: &IpRepr, ip_payload: &'frame [u8]
    ) -> bool {
    ...
    fn process_ipv6<'frame, T: AsRef<[u8]>>(
        ...
        match ipv6_repr.next_header {
            IpProtocol::Icmpv6 => self.process_icmpv6(sockets, ip_repr, ip_payload),
    ...
    fn process_ipv4<'frame, T: AsRef<[u8]>>(
        ...
        if !ipv4_repr.dst_addr.is_broadcast() && !ipv4_repr.dst_addr.is_multicast()
    ...
    fn process_icmpv6<'frame>(
        &self, _sockets: &mut SocketSet, ip_repr: IpRepr, ip_payload: &'frame [u8]
    ) -> Result<Packet<'frame>> {
    ...
    fn process_icmpv4<'frame>(
        ...
        for mut icmp_socket in _sockets.iter_mut().filter_map(IcmpSocket::downcast) {
            if !icmp_socket.accepts(&ip_repr, &icmp_repr, &checksum_caps) {
                continue;
            }
    ...
    fn process_udp<'frame>(
        &self, sockets: &mut SocketSet, ip_repr: IpRepr, ip_payload: &'frame [u8]
    ) -> Result<Packet<'frame>> {
    ...
    fn process_tcp<'frame>(
        &self, sockets: &mut SocketSet, timestamp: Instant, ip_repr: IpRepr,
        ip_payload: &'frame [u8],
    ) -> Result<Packet<'frame>> {
        let (src_addr, dst_addr) = (ip_repr.src_addr(), ip_repr.dst_addr());
    ...
    fn dispatch_ethernet<Tx, F>(
        &mut self, tx_token: Tx, timestamp: Instant, buffer_len: usize, f: F
    ) -> Result<()>
    where
        Tx: TxToken,
    ...
    fn in_same_network(&self, addr: &IpAddress) -> bool {
        self.ip_addrs.iter().find(|cidr| cidr.contains_addr(addr)).is_some()
    }
    ...
    fn has_neighbor<'a>(&self, addr: &'a IpAddress, timestamp: Instant) -> bool {
        match self.route(addr) {
            Ok(routed_addr) => self.neighbor_cache.lookup_pure(&routed_addr, timestamp).is_some(),
            Err(_) => false,
        }
    }

    fn lookup_hardware_addr<Tx>(
        &mut self, tx_token: Tx, timestamp: Instant, src_addr: &IpAddress, dst_addr: &IpAddress
        ...
        Tx: TxToken,
    {
        if dst_addr.is_multicast() {
            ...
            let hardware_addr = match dst_addr {
                ...
                &IpAddress::Ipv4(_addr) => {
                    Some(EthernetAddress::from_bytes(&[0x01, 0x00, 0x5e, b[1] & 0x7F, b[2], b[3]]))
                }
                #[cfg(feature = "proto-ipv6")]
                &IpAddress::Ipv6(_addr) => {
                    Some(EthernetAddress::from_bytes(&[0x33, 0x33, b[12], b[13], b[14], b[15]]))
                }
        ...
        match (src_addr, dst_addr) {
            ...
            (&IpAddress::Ipv4(src_addr), IpAddress::Ipv4(dst_addr)) => {
                net_debug!("address {} not in neighbor cache, sending ARP request", dst_addr);
    ...
    fn dispatch_ip<Tx, F>(
        &mut self, tx_token: Tx, timestamp: Instant, ip_repr: IpRepr, f: F
    ) -> Result<()>
    where
        Tx: TxToken,
...
mod test {
    ...
    #[cfg(feature = "proto-ipv6")]
    use wire::{Icmpv6ParamProblem, Icmpv6Repr};
    ...
    fn test_no_icmp_to_broadcast() {
        ...
        // broadcast address
        assert_eq!(
            iface.inner.process_ipv4(&mut socket_set, Instant::from_millis(0), &frame),
            Ok(Packet::None)
        );
    }
    ...
    fn test_icmp_error_port_unreachable() {
        ...
        // Ensure that the unknown protocol triggers an error response.
        // And we correctly handle no payload.
        assert_eq!(iface.inner.process_udp(&mut socket_set, ip_repr, data), Ok(expected_repr));
        ...
        assert_eq!(
            iface.inner.process_udp(&mut socket_set, ip_repr, packet_broadcast.into_inner()),
            Ok(Packet::None)
        );
    }
    ...
    fn test_handle_udp_broadcast() {
        ...
        assert_eq!(
            iface.inner.process_udp(&mut socket_set, ip_repr, packet.into_inner()),
            Ok(Packet::None)
        );
    ...
    fn test_icmpv4_reply_size() {
        ...
        assert_eq!(
            expected_ipv4_repr.buffer_len() + expected_icmpv4_repr.buffer_len(),
            IPV4_MIN_MTU
        );
        // The expected packet and the generated packet are equal
        assert_eq!(
            iface.inner.process_udp(&mut socket_set, ipv4_repr.into(), payload),
    ...
    fn test_icmpv4_socket() {
        ...
        assert_eq!(
            iface.inner.process_icmpv4(&mut socket_set, ip_repr, icmp_data),
...

        {
            ...
            assert_eq!(
                socket.recv(),
                Ok((&icmp_data[..], IpAddress::Ipv4(Ipv4Address::new(0x7f, 0x00, 0x00, 0x02))))
            );
        }
    }
    ...
    fn test_icmpv6_nxthdr_unknown() {
        ...
        assert_eq!(
            iface.inner.process_ipv6(&mut socket_set, Instant::from_millis(0), &frame),
