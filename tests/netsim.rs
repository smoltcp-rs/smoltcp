...
use smoltcp::phy::{
    self, ChecksumCapabilities, Device, DeviceCapabilities, Medium, PcapMode, PcapWriter,
};
...
fn run_test(case: TestCase) -> f64 {
    ...
    let mut device_a = QueueDevice::new(&queue_a_to_b, &queue_b_to_a, Medium::Ethernet);
    let mut device_b = QueueDevice::new(&queue_b_to_a, &queue_a_to_b, Medium::Ethernet);

    let pcap_writer = File::create("out.pcap").expect("cannot open file");
    let pcap_writer = BufWriter::with_capacity(65536, pcap_writer);
    let mut device_b = PcapWriter::new(device_b, pcap_writer, PcapMode::Both);
