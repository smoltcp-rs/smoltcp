...
use smolutils::time::{Duration, Instant};
use smolwire::{self, IpAddress, IpProtocol, IpRepr, UdpRepr};
use smolwire::{
...
const MDNS_IPV6_ADDR: IpAddress = IpAddress::Ipv6(smolwire::Ipv6Address([
    0xff, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xfb,
]));

#[cfg(feature = "proto-ipv4")]
const MDNS_IPV4_ADDR: IpAddress = IpAddress::Ipv4(smolwire::Ipv4Address([224, 0, 0, 251]));
...
impl<'a> Socket<'a> {
    ...
    pub(crate) fn process(
        ...
        let p = match DnsPacket::new_checked(payload) {
            Ok(x) => x,
            Err(_) => {
        ...
        if p.opcode() != DnsOpcode::Query {
            net_trace!("unwanted opcode {:?}", p.opcode());
            return;
        }

        if !p.flags().contains(DnsFlags::RESPONSE) {
        ...
        for q in self.queries.iter_mut().flatten() {
            if let State::Pending(pq) = &mut q.state {
                ...
                let (mut payload, question) = match DnsQuestion::parse(payload) {
                    Ok(x) => x,
                    Err(_) => {
                ...
                for _ in 0..p.answer_record_count() {
                    let (payload2, r) = match DnsRecord::parse(payload) {
                        Ok(x) => x,
                        Err(_) => {
                    ...
                    match r.data {
                        ...
                        DnsRecordData::Other(type_, data) => {
        ...
        F: FnOnce(&mut Context, (IpRepr, UdpRepr, &[u8])) -> Result<(), E>,
    {
        ...
        for q in self.queries.iter_mut().flatten() {
            if let State::Pending(pq) = &mut q.state {
                ...
                repr.emit(&mut DnsPacket::new_unchecked(payload));
...
fn eq_names<'a>(
    mut a: impl Iterator<Item = smolwire::Result<&'a [u8]>>,
    mut b: impl Iterator<Item = smolwire::Result<&'a [u8]>>,
) -> smolwire::Result<bool> {
    loop {
        match (a.next(), b.next()) {
            // Handle errors
...
fn copy_name<'a, const N: usize>(
    dest: &mut Vec<u8, N>,
    name: impl Iterator<Item = smolwire::Result<&'a [u8]>>,
) -> Result<(), smolwire::Error> {
    dest.truncate(0);

    for label in name {
        let label = label?;
        dest.push(label.len() as u8).map_err(|_| smolwire::Error)?;
        dest.extend_from_slice(label).map_err(|_| smolwire::Error)?;
    }

    // Write terminator 0x00
    dest.push(0).map_err(|_| smolwire::Error)?;
