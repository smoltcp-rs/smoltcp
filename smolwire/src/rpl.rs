...
impl<'p> Repr<'p> {
    ...
    pub fn emit<T: AsRef<[u8]> + AsMut<[u8]> + ?Sized>(&self, packet: &mut Packet<&mut T>) {
        packet.set_msg_type(crate::icmpv6::Message::RplControl);
...
pub mod options {
    ...
    pub enum Repr<'p> {
        ...
        RplTarget {
            prefix_length: u8,
            prefix: crate::Ipv6Address, // FIXME: this is not the correct type, because the
        ...
        pub fn parse<T: AsRef<[u8]> + ?Sized>(packet: &Packet<&'p T>) -> Result<Self> {
            match packet.option_type() {
                ...
                OptionType::RplTarget => Ok(Repr::RplTarget {
                    ...
                    prefix: crate::Ipv6Address::from_bytes(packet.target_prefix()),
                }),
